...
class ResponseController extends Controller
{
    ...
    {
        try {
            $response = $this->res->create($request->except('_token'));
            return redirect()->route('data.complaint');
        } catch (\Exception $e) {
    ...
    {
        try {
            $update = $this->res->where('id', $id)->update($request->except('_method', '_token'));
            $response = $this->res->find($id);
            return redirect()->route('data.respond');
        } catch (\Exception $e) {
            return $this->onError($e);
        }
    }
